    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not user or user.role not in roles:
                return jsonify({'error': 'ليس لديك صلاحية للوصول لهذه الصفحة'}), 403
...
def change_password():
    ...
    try:
        ...
        if len(new_password) < 3:
            return jsonify({'error': 'كلمة المرور يجب أن تكون 3 أحرف على الأقل'}), 400
        ...
        # التحقق من عدم استخدام كلمة المرور المبدئية مرة أخرى
        if new_password == '123':
            return jsonify({'error': 'لا يمكن استخدام كلمة المرور المبدئية مرة أخرى'}), 400
