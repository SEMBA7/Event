...
def get_users():
    ...
    try:
        ...
        for user in users:
            ...
            user_dict['creator_name'] = user.creator.username if user.creator else None
...
def update_user(user_id):
    ...
    try:
        ...
        if user.username == 'Gon' and user_id != session['user_id']:
            return jsonify({'error': 'لا يمكن تعديل المستخدم الرئيسي'}), 403
        ...
        log_entry = Log(
            ...
            target_type=LOG_TARGETS['USER'],
            target_id=user.id,
            details=f'تم تحديث المستخدم: {old_username} -> {new_username}, الدور: {old_role} -> {new_role}',
            created_by=session['user_id']
        )
        db.session.add(log_entry)
        db.session.commit()
...
def reset_user_password(user_id):
    ...
    try:
        ...
        # إعادة تعيين كلمة المرور إلى 123
...
def delete_user(user_id):
    ...
    try:
        ...
        if user.username == 'Gon':
            return jsonify({'error': 'لا يمكن حذف المستخدم الرئيسي'}), 403
